...
@torch.no_grad()
def generate(args, g_ema, device, mean_latent,l1,l2,r):

    with torch.no_grad():
        g_ema.eval()
        
        for i in tqdm(range(args.pics)):
            latent = (i/args.pics)*(l1-l2) 
            latent = latent +r
            ...
            utils.save_image(
                sample,
                f"output5/{str(i).zfill(6)}.png",
                nrow=1,
                normalize=True,
                range=(-1, 1),
            )
        create_gif("output5",f"pca_test/test2.gif")
...
if __name__ == "__main__":
    ...
    r = data[args.start]
    checkpoint1 =data_pca[args.goal]- data_pca[args.start] 
    checkpoint1= pca.inverse_transform(checkpoint1)
    
    if(args.n_comp>1):
        pca = PCA(n_components=args.n_comp-1)
        pca.fit(data)
        data_pca = pca.transform(data)
        checkpoint2 = data_pca[args.goal]- data_pca[args.start] 
        checkpoint2=pca.inverse_transform(checkpoint2)
    else:
        checkpoint2=0
    ...
    generate(args, g_ema, device, mean_latent,checkpoint1,checkpoint2,r)